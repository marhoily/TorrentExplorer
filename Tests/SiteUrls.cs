...
public static class SiteUrls
{
    ...
    {
        var html = await http.Get($"topics/fantasy/{page:D3}", $"forum/viewforum.php?f=2387&start={page*50}");
        return html.ParseHtml();
    }
    public static async Task<HtmlNode> DownloadRussianFantasyTopic(this Http http, int topicId)
    {
        var html = await http.Get($"topic/fantasy/{topicId:D8}", $"forum/viewtopic.php?t={topicId}");
        return html.ParseHtml();
    }
}